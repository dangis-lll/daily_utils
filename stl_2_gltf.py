...
'''
    ...
    vertices = {}
    indices = []
    filename = os.path.splitext(os.path.split(path_to_stl)[1])[0]

    if not is_binary:

        out_bin = os.path.join(out_path, "{}.bin".format(filename))
        out_gltf = os.path.join(out_path, "{}.gltf".format(filename))
    else:
        out_bin = os.path.join(out_path, "{}.glb".format(filename))
    ...
    if is_binary:
        ...
        out_bin_uir = '"uri": "{}.bin",'.format(filename)
...
if __name__ == '__main__':
    ...
    if terminal:
        if len(sys.argv) < 3:
            print("use it like python3 stl_to_gltf.py /path/to/stlfolder /path/to/gltffolder")
            print("or          python3 stl_to_gltf.py /path/to/stlfolder /path/to/glbfile -b")
            sys.exit(1)
        ...
        path_to_stl = 'gltf_file/stl'
        out_path = "gltf_file/gltf"
        is_binary = 0


    for i in os.listdir(path_to_stl):
        data = os.path.join(path_to_stl,i)
        stl_to_gltf(data, out_path, is_binary)